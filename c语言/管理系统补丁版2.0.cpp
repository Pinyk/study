...
void change()
{
	...
	if(node)
	{
		...
		do{
			...
			{
				case '1':
					str=modi_field("身高",node->height,3);
					if(str!=NULL)
					{
						strcpy(node->height,str);
						free(str);
					}
				case '2':
					str=modi_field("球技",node->qiuji,10);
					if(str!=NULL)
					{
				...
				case '3':
					str=modi_field("球衣编号",node->num,3);
					if(str!=NULL)
					{
						strcpy(node->num,str);
						free(str);
